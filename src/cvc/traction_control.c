...
void Traction_Calculate_SlipRatio() {
    ...
    int16_t front_left_speed = (int16_t)(CVC_data[SENSOR_LEFT_WHEELSPEED]);
    int16_t front_right_speed = (int16_t)(CVC_data[SENSOR_RIGHT_WHEELSPEED]);
    int16_t rear_left_speed = (int16_t)CVC_data[INVERTER1_MOTOR_SPEED_HS];
    int16_t rear_right_speed = (int16_t)CVC_data[INVERTER2_MOTOR_SPEED_HS];
    ...
    if (rear_left_speed <= MIN_REAR_SPEED || rear_right_speed <= MIN_REAR_SPEED) {
        ...
        CVC_data[CVC_SLIP_RATIO_LEFT] = (uint64_t)(((rear_left_speed - front_left_speed) / rear_left_speed) * RPM_SCALE_FACTOR);
        CVC_data[CVC_SLIP_RATIO_RIGHT] = (uint64_t)(((rear_right_speed - front_right_speed) / rear_right_speed) * RPM_SCALE_FACTOR);
    }
}