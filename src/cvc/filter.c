...
/* --------------------------------------- Declare Global Variables and Local
 * Functions -----------------------------------*/
...
// static float IIR_raw_impulse[FILTER_LENGTH + 1] = {0.134793, -0.39975544,
// 0.26500209, 0.26500209, -0.39975544, 0.134793};

// // a coefficients (denominator, excluding a[0] which is normalized to 1)
// static float IIR_filtered_impulse[FILTER_LENGTH] = {-4.30378556, 7.42328477,
// -6.41349104, 2.77428788, -0.48021674};
...
void Filter_Configure() {
    // Initialize FIR filter
    FIR_filter_init(&left_IIR);
    FIR_filter_init(&right_IIR);
}
...
float IIR_filter_update(IIR_filter* filter, int32_t input) {
    ...
    // Reset index to 0 when it reaches end of buffer (circular buffer
    // implementation)
    ...
    // Process b coefficients (FIR portion) - traverse buffer in reverse
    // chronological order
    ...
    // Process a coefficients (IIR feedback portion) - traverse buffer in
    // reverse order
...
void Filter_Speed(IIR_filter* left_IIR, IIR_filter* right_IIR) {
    if (Inverter1_HS_Flag) {
        ...
        CVC_data[INVERTER1_MOTOR_SPEED_HS_FILTERED] = filtered;  // Update CVC_data with left inverter filtered speed
                                                                 // (RPM)
    ...
    if (Inverter2_HS_Flag) {
        ...
        CVC_data[INVERTER2_MOTOR_SPEED_HS_FILTERED] = filtered;  // Update CVC_data with right inverter filtered speed
                                                                 // (RPM)
